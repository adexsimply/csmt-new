<div class="col-sm-12">
	@if($attendances)
		...
		<form method="post" onsubmit="oisForm(event)" action="{{url('clinic/update')}}">
					...
					@foreach($attendances as $x => $student)
						<tr>
							...
							<td>
								...
								@if($student->status == 1)
									<input checked="" class="radio-lg" type="checkbox" name="status[{{$x}}]" value="1">
								@else
									<input type="checkbox" class="radio-lg" name="status[{{$x}}]" value="1">
								@endif
							</td>
						</tr>
					@endforeach
