<div class="col-sm-12">
	@if($students)
		...
		<input data-type="purple" data-title="Previously marked" data-size="xl" type="date" data-url="{{url('assignment/hostel/show/'.$aagc_id.'/'.$session_id.'/'.$term_id.'/'.$category_id)}}" onchange="viewAttendance(event)" class="form-control" />
		<hr />

		<h3>Mark new </h3>
		<form method="post" onsubmit="oisForm(event)" action="{{url('assignment/hostel/store')}}">
			...
			<table class="table table-hover table-padded">
				<thead>
					...
					<th>Name</th>
					<th>Student ID</th>
					<th class="text-center"><input type="radio" onclick="mark_early()" class="radio-lg" name="mark_all">Completed</th>
					<th class="text-center"><input type="radio" onclick="mark_late()" class="radio-lg" name="mark_all">Not Completed</th>
				</thead>
