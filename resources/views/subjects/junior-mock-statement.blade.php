...
.table-bordered > tfoot > tr > td {
 ...
 body 
		...
		<div class="main">
				...
				<tbody>
					<tr>
						...
						<td class="input" style="padding:3px;">
							<strong>Examination&nbsp;No:</strong>&nbsp;<strong class="blue-st">{{$student->admission_no}}</strong>
						</td>
			        ...
			        <tbody class="input">
			        	<?php $credits = 0; $passes=0; $x=1; ?>
			         	@foreach($subjects as $subject)
			         		<?php $score = App\Junior_mock::score($student->id,$subject->id)->score; ?>
			         		<tr>
					            <th style="">{{$x}}</th>
					            <td class="blue-subj">{{$subject->name}}</td>
					            <td class="blue-subj" style="padding-left:3%;" >{{App\Junior_mock::grade($score)}}</strong></td>
					            <td>{{App\Junior_mock::remark($score)}}</td>
					          </tr>
					        <tr>
			...
			<div class=""></div>
					...
					<div style="width:66%;font-size:20px;float:left;margin-top:5px;">					
						<p>Remarks: <span class="blue-com">{{$student->junior_mock_details->remark}}</span><br/>
	...
	@else
		<h1 class="text-center text-danger">No score found!</h1>
	@endif
</body>
</html>