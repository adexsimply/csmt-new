		...
		<div class="main">
			        ...
			        <tbody class="input">
			        	<?php $credits = 0; $passes=0; $x=1; ?>
			         	@foreach($subjects as $subject)
			         		<?php $score = App\Senior_mock::score($student->id,$subject->id)->score; ?>
			         		<tr>
					            <th style="">{{$x}}</th>
					            <td class="blue-subj">{{$subject->name}}</td>
					            <td class="blue-subj" style="padding-left:3%;" >{{App\Senior_mock::grade($score)}}</strong></td>
					            <td>{{App\Senior_mock::remark($score)}}</td>
					          </tr>
					        <tr>
			...
			<div class=""></div>
					...
					<div style="width:66%;font-size:20px;float:left;margin-top:5px;">					
						<p>Remarks: <span class="blue-com">{{$student->senior_mock_details->remark}}</span><br/>
