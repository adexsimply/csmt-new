...
{	
	private $session_id, $term_id,$username,$apikey,$name;
    private $table = 'sms_pin';

	function __construct(){
		$Aagc = new Aagc;
		$active = $Aagc::active();
        $active_st = $Aagc::active_session_term();
		$this->session_id = $active->session_id;
		$this->term_id = $active->term_id;
        $this->term_name = $active_st->term_name;
        /*// CSMT API
    ...
    private function sendBulkSMS($sender="CSMT SEC SC",$message,$phone){
        return [
            "SMS" => [
                "auth" => [
	...
	public function pin(){
		...
		return view('sms.pins',compact('pins','term_name'))->render();
	}
    ...
    public function sendPinToParent(Request $request){
        ...
        foreach($parents as $x => $parent){
            ...
            $message = "Dear Parent/Guardian, kindly visit https://www.csmtschools.com/portal to check your child's ".$term_name." result. Use the details provided below for validation. Hard copies of the result sheet are available on request and payment in the school. Thank you. PIN: ".$parent->pin." Username ".$parent->admission_no." result. Signed: MGT - CSMT SCHOOLS ";
    ...
    public function sendMessageToParent(Request $request){
        ...
        $parents = DB::table('parents')
                        ...
                        ->where($condition)
                        ->select(DB::raw('DISTINCT parents.phone1 as phone, parents.id'))
                        ->get();
        ...
        return response($this->sendBulkSMS($request->sender,$request->message,$phone));
    ...
    public function generatePin(Request $request){
    	...
    	$students = DB::table('aagc_session_student')
    					...
    					->where($condition)
    					->select('student_id','students.student_category_id')
    					->get();
