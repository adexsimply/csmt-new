<div class="col-sm-12">
	<?php if($students): ?>
		...
		<input data-type="purple" data-title="Previously marked" data-size="xl" type="date" data-url="<?php echo e(url('punctuality/classroom/show/'.$aagc_id.'/'.$session_id.'/'.$category_id.'/'.$term_id)); ?>" onchange="viewAttendance(event)" class="form-control" />
		<hr />

		<h3>Mark new </h3>
		<form method="post" onsubmit="oisForm(event)" action="<?php echo e(url('punctuality/classroom/store')); ?>">
				...
				<tbody>
					<?php $__currentLoopData = $students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $x => $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
						<tr>
							...
							<td class="text-center"><input type="radio" id="early" required class="radio-lg early" name="status[<?php echo e($x); ?>]" value="1"></td>

							<td class="text-center"><input type="radio" required class="radio-lg late" name="status[<?php echo e($x); ?>]" value="0"></td>

							<td class="text-center"><input type="radio" required class="radio-lg perm" name="status[<?php echo e($x); ?>]" value="2"></td>
						</tr>
